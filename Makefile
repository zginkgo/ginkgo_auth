PROJECT_NAME := "ginkgo_auth"
MAIN_FILE_PAHT := "main.go"
PKG := "github.com/zginkgo/ginkgo_auth"
IMAGE_PREFIX := "ginkgo_auth"

MOD_DIR := $(shell go env GOPATH)/pkg/mod
PKG_LIST := $(shell go list ${PKG}/... | grep -v /vendor/ | grep -v redis)
GO_FILES := $(shell find . -name '*.go' | grep -v /vendor/ | grep -v _test.go)

MCUBE_MODULE := "github.com/zginkgo/ginkgo_pkg"
...
run: install dep ## Run Server
	@go run main.go start -f etc/ginkgo_auth_dev.toml
...
pb: ## Copy mcube protobuf files to common/pb
	@mkdir -pv common/pb/github.com/zginkgo/ginkgo_pkg/pb
	@sudo cp -r ${MCUBE_PKG_PATH}/pb/* common/pb/github.com/zginkgo/ginkgo_pkg/pb
	@sudo rm -rf common/pb/github.com/zginkgo/ginkgo_pkg/pb/*/*.go
